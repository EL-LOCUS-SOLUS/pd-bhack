...
-- ─────────────────────────────────────
-- Does compute_figure's answer actually equal the requested value?
-- If not, the value needs to be split into a tied chain.
local function figure_matches(value, dot_level, figure, min_figure)
    if not figure or figure <= 0 then
        return false
    end
    local base = utils.floor_pow2(value)
    if base == 0 then
        return false
    end
    local reconstructed = base
    local add = base / 2
    for _ = 1, (dot_level or 0) do
...
-- ─────────────────────────────────────
-- Greedy: largest power-of-two chunk, then recurse on the remainder.
local function split_pow2(value)
    local pieces = {}
    local remaining = value
    local guard = 0
    while remaining > 1e-9 and guard < 64 do
        guard = guard + 1
...
function Measure:expand_level(rhythms, container_duration, parent_tuplet, measure_min_figure, parent_min_figure)
    ...
    for _, entry in ipairs(rhythms) do
        ...
        elseif type(entry) == "number" then
            ...
            local dot, fig = rhythm.compute_figure(mag, parent_min_figure)
            if figure_matches(mag, dot, fig, parent_min_figure) then
                self:append_value_entry(entry, nil, parent_tuplet, total, container_duration, parent_min_figure, false)
            else
                local pieces = split_pow2(mag)
                if #pieces <= 1 then
                    self:append_value_entry(entry, nil, parent_tuplet, total, container_duration, parent_min_figure, false)
                else
                    for i, p in ipairs(pieces) do
                        local tied = (i < #pieces) and (sign > 0)
                        self:append_value_entry(sign * p, nil, parent_tuplet, total, container_duration, parent_min_figure, tied)
                    end
                end
            end
        elseif type(entry) == "string" then
            ...
            if #pieces <= 1 then
                ...
                for i, p in ipairs(pieces) do
                    local tied = true
                    self:append_value_entry(p, nil, parent_tuplet, total, container_duration, parent_min_figure, tied)
                end
            end
        else
